...
10. 检查设备 ID、输入模式和日志边界是否同步：新建设备 ID 四段规则、`input_mode`/`increment_rule`、run START/END 边界、trace 真实 `action.value`。
11. 检查旧模型概念是否残留在主路径：复杂绑定、手工结果声明、复杂判断对象、多模式识别、独立等待动作或截图校验动作。
