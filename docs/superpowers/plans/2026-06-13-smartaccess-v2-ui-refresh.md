...
**目标：** 将 SmartAccess v2 PyQt 客户端重构为现代浅色卡片式工作台，同时保持现有业务逻辑不变。
...
**技术栈：** Python 3.11+、PyQt6、Qt QSS、现有 `smartaccess_v2` 桌面模块。

---

## 文件结构

- 修改：`src/smartaccess_v2/desktop/shell/theme.py`
  - 负责全局配色、按钮、表单、导航、卡片、表格和滚动条 QSS。
- 修改：`src/smartaccess_v2/desktop/shell/main_window.py`
  - 负责主窗口浅灰背景、顶部栏、导航图标预留位和右侧上下文栏。
- 新建：`src/smartaccess_v2/desktop/widgets/cards.py`
  - 负责创建统一卡片容器，避免页面里重复设置 `QFrame#Card`、padding 和 spacing。
- 新建：`src/smartaccess_v2/desktop/widgets/table_style.py`
  - 负责统一 `QTableWidget` 行高、网格、表头、选择和内嵌控件高度。
- 修改：`src/smartaccess_v2/desktop/pages/calibration_page.py`
  - 负责设备接入页面的表单、窗口列表、截图画布和锚点表格卡片化。
- 修改：`src/smartaccess_v2/desktop/pages/workflow_page.py`
  - 负责工作流列表、元数据表单、AI 输入、步骤表和结果区卡片化。
- 修改：`src/smartaccess_v2/desktop/widgets/anchor_table.py`
  - 负责锚点表行高、按钮、下拉框、复选框和表格边界。
- 修改：`src/smartaccess_v2/desktop/widgets/workflow_step_table.py`
  - 负责步骤表行高、列内控件高度和行操作按钮。
- 修改：`src/smartaccess_v2/desktop/widgets/condition_editor.py`
  - 负责 OCR 条件编辑器内控件最小高度和对象名。
- 修改：`src/smartaccess_v2/desktop/widgets/roi_canvas.py`
  - 负责截图画布背景、边框和对象名。
- 修改：`src/smartaccess_v2/desktop/widgets/timeline.py`
  - 负责运行监控时间线表格复用统一表格样式。
- 修改：`src/smartaccess_v2/desktop/pages/template_page.py`
  - 负责模板页过滤区和表格卡片化，保证全局表格样式一致。
- 修改：`src/smartaccess_v2/desktop/pages/dashboard_page.py`
...
```powershell
python -c "from smartaccess_v2.desktop.shell.theme import build_qss; qss=build_qss(); assert 'QFrame#Card' in qss; assert 'QTableWidget::item' in qss; assert '#F5F7FA' in qss; print('theme qss ok')"
```
...
```powershell
git add src/smartaccess_v2/desktop/shell/theme.py
...
创建 `src/smartaccess_v2/desktop/widgets/cards.py`：
...
创建 `src/smartaccess_v2/desktop/widgets/table_style.py`：
...
```powershell
python -c "from smartaccess_v2.desktop.widgets.cards import create_card; from smartaccess_v2.desktop.widgets.table_style import configure_data_table; print(create_card.__name__, configure_data_table.__name__)"
```
...
```powershell
git add src/smartaccess_v2/desktop/widgets/cards.py src/smartaccess_v2/desktop/widgets/table_style.py
...
```powershell
$env:QT_QPA_PLATFORM='offscreen'; python -c "from PyQt6.QtWidgets import QApplication; from smartaccess_v2.bootstrap.runtime import build_runtime_facade; from smartaccess_v2.desktop.shell.main_window import MainWindow; from smartaccess_v2.desktop.shell.theme import apply_theme; from smartaccess_v2.shared.config.settings import AppSettings; app=QApplication([]); apply_theme(app); settings=AppSettings(workspace_dir='tmp-ui-smoke'); facade=build_runtime_facade(settings); window=MainWindow(settings, facade=facade); assert window._nav.count()==5; assert window._stack.count()==5; assert window._nav.iconSize().width()==18; print('v2 shell ok')"
```
...
```powershell
git add src/smartaccess_v2/desktop/shell/main_window.py
...
```powershell
git add src/smartaccess_v2/desktop/widgets/anchor_table.py src/smartaccess_v2/desktop/widgets/workflow_step_table.py src/smartaccess_v2/desktop/widgets/condition_editor.py src/smartaccess_v2/desktop/widgets/timeline.py
...
```powershell
$env:QT_QPA_PLATFORM='offscreen'; python -c "from PyQt6.QtWidgets import QApplication; from smartaccess_v2.bootstrap.runtime import build_runtime_facade; from smartaccess_v2.desktop.pages.calibration_page import CalibrationPage; from smartaccess_v2.desktop.shell.theme import apply_theme; from smartaccess_v2.shared.config.settings import AppSettings; app=QApplication([]); apply_theme(app); settings=AppSettings(workspace_dir='tmp-ui-smoke'); page=CalibrationPage(build_runtime_facade(settings)); assert page._table.verticalHeader().defaultSectionSize()>=40; assert page._canvas.objectName()=='RoiCanvas'; print('v2 calibration ok')"
```
...
```powershell
git add src/smartaccess_v2/desktop/pages/calibration_page.py src/smartaccess_v2/desktop/widgets/roi_canvas.py
...
```powershell
$env:QT_QPA_PLATFORM='offscreen'; python -c "from PyQt6.QtWidgets import QApplication; from smartaccess_v2.bootstrap.runtime import build_runtime_facade; from smartaccess_v2.desktop.pages.workflow_page import WorkflowPage; from smartaccess_v2.desktop.shell.theme import apply_theme; from smartaccess_v2.shared.config.settings import AppSettings; app=QApplication([]); apply_theme(app); settings=AppSettings(workspace_dir='tmp-ui-smoke'); page=WorkflowPage(build_runtime_facade(settings)); assert page._steps.verticalHeader().defaultSectionSize()>=40; assert page._ai_prompt.objectName()=='PromptEditor'; print('v2 workflow ok')"
```
...
```powershell
git add src/smartaccess_v2/desktop/pages/workflow_page.py
...
```powershell
$env:QT_QPA_PLATFORM='offscreen'; python -c "from PyQt6.QtWidgets import QApplication; from smartaccess_v2.bootstrap.runtime import build_runtime_facade; from smartaccess_v2.desktop.pages.dashboard_page import DashboardPage; from smartaccess_v2.desktop.pages.template_page import TemplatePage; from smartaccess_v2.desktop.shell.theme import apply_theme; from smartaccess_v2.shared.config.settings import AppSettings; app=QApplication([]); apply_theme(app); settings=AppSettings(workspace_dir='tmp-ui-smoke'); facade=build_runtime_facade(settings); template=TemplatePage(facade); dashboard=DashboardPage(facade); assert template._table.verticalHeader().defaultSectionSize()>=40; assert dashboard._runs.verticalHeader().defaultSectionSize()>=40; print('v2 secondary pages ok')"
```
...
```powershell
git add src/smartaccess_v2/desktop/pages/template_page.py src/smartaccess_v2/desktop/pages/dashboard_page.py
...
```powershell
python -m compileall src/smartaccess_v2/desktop
```
...
```powershell
git add src/smartaccess_v2/desktop
