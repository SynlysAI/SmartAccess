...
在不改动旧 `src/smartaccess` 实现的前提下，新建 `src/smartaccess`，用更轻、更清晰的结构完整复刻 SmartAccess 当前功能，并修复已知界面、校准、工作流、运行和日志问题。

允许新增根目录启动脚本 `run_desktop.py`。除该入口外，产品实现代码限定在 `src/smartaccess`。
...
- `python run_desktop.py` 能启动 SmartAccess 浅色客户端。
- 默认数据写入 `workspace_v2`。
